...
parser.add_argument(
    "-i", action="store", required=True, help="Path to input log files/figures."
)
parser.add_argument(
    "--tsv-out",
    type=str,
...
parser.add_argument(
    "--benchmark", type=str, required=True, help="Name of the benchmark"
)
...
tsv_path = (
    ...
    if (args.tsv_out is not None)
        ...
        filepath, "mlperf_summary_" + str(find_max_id(filepath, "summary")) + ".ods"
    )
)
    ...
    for file in files:
        if file.endswith(".tsv") and not file.endswith("logs.tsv"):
            ...
            if name in df["name"].values:
                df.loc[(df["name"] == name), "timestamp"] = metrics_tmp.loc[
                    0, "timestamp"
                ]
            ...
            if name in df["name"].values:
                for column in metrics_tmp.columns:
                    try:
                        if column == "qps":
                            df.loc[df["name"] == name, "qps"] += metrics_tmp.loc[
                                0, "qps"
                            ]
                        elif column == "mean":
                            df.loc[df["name"] == name, "mean"] += (
                                float(metrics_tmp.loc[0, "mean"])
                                / benchmarks_count[name]
                            )
                        elif column == "time":
                            if (
                                metrics_tmp.loc[0, "time"]
                                > df.loc[df["name"] == name, "time"].values[0]
                            ):
                                df.loc[df["name"] == name, "time"] = metrics_tmp.loc[
                                    0, "time"
                                ]
                        elif column == "system_latency":
                            if (
                                metrics_tmp.loc[0, "system_latency"]
                                > df.loc[df["name"] == name, "system_latency"].values[0]
                            ):
                                df.loc[df["name"] == name, "system_latency"] = round(
                                    metrics_tmp.loc[0, "system_latency"], 4
                                )
                        elif column == "acc":
                            df.loc[df["name"] == name, "acc"] += (
                                float(metrics_tmp.loc[0, "acc"].strip("%")) / 100
                            ) / benchmarks_count[name]
                        elif column == "queries":
                            df.loc[df["name"] == name, "queries"] += metrics_tmp.loc[
                                0, "queries"
                            ]
                        elif column == "tiles":
                            g = [
                                x.split(":")
                                for x in metrics_tmp.loc[0, "tiles"].split(",")
                            ]
                            tmp = {}
                            for x1, x2 in g:
                                if (
                                    x1 not in df.columns
                                    or df.loc[df["name"] == name, x1]
                                    .isnull()
                                    .values.any()
                                ):
                                    df.loc[df["name"] == name, x1] = round(
                                        float(x2) / benchmarks_count[name], 4
                                    )
                                else:
                                    df.loc[df["name"] == name, x1] += round(
                                        float(x2) / benchmarks_count[name], 4
                            ...
                            df.loc[df["name"] == name, "mAP"] += (
                                float(metrics_tmp.loc[0, "mAP"].strip("%")) / 100
                            ) / benchmarks_count[name]
                        elif column != "name":
                            df.loc[df["name"] == name, column] = metrics_tmp.loc[
                                0, column
                            ]
                    except:
                        pass
                ...
                try:
                    df.loc[len(df.index) - 1, "system_latency"] = round(
                        df.loc[len(df.index) - 1, "system_latency"], 4
                    )
                except:
                    pass
                try:
                    df.loc[len(df.index) - 1, "acc"] = (
                        float(df.loc[len(df.index) - 1, "acc"].strip("%")) / 100
                    ) / benchmarks_count[name]
                except:
                    pass
                try:
                    df.loc[len(df.index) - 1, "mean"] = (
                        float(df.loc[len(df.index) - 1, "mean"])
                        / benchmarks_count[name]
                    )
                except:
                    pass
                try:
                    df.loc[len(df.index) - 1, "mAP"] = (
                        float(df.loc[len(df.index) - 1, "mAP"].strip("%")) / 100
                    ) / benchmarks_count[name]
                except:
                    pass
                try:
                    ...
                    for x1, x2 in g:
                        if (
                            x1 not in df.columns
                            or df.loc[df["name"] == name, x1].isnull().values.any()
                        ):
                            df.loc[df["name"] == name, x1] = round(
                                float(x2) / benchmarks_count[name], 4
                            )
...
for row in range(len(df)):
    ...
    if name == "total":
        df.loc[row, "execution_speed"] = round(
            total_benchmark_count / df.loc[row, "system_latency"], 4
        )
        continue
    df.loc[row, "execution_speed"] = round(
        benchmarks_count[name] / df.loc[row, "system_latency"], 4
    )
